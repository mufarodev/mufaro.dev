      ...
      {experiences.map((experience) => {
        ...
        return (
          <motion.div
            variants={card}
            className={cn(
              "relative backdrop-blur-xl bg-black/25 border border-white/10 rounded-2xl shadow-2xl",
              data.url ? "hover:border-white/20 hover:bg-black/30 transition-all duration-300 cursor-pointer" : ""
            )}
            key={experience.slug}
            onClick={() => {
              if (data.url) {
          ...
          >
            ...
            <div className="p-6 flex justify-between relative">
              <div className="flex flex-col gap-2">
                ...
                {data.url && (
                  <p className="text-sm text-white opacity-75">{data.url}</p>
                )}
                ...
                <div className="flex gap-2 items-center">
                  ...
                  {data.flag &&
                    <span
                      className={cn(`px-2 py-[1px] rounded-full uppercase font-semibold text-xs text-white/70 w-fit bg-white/10 border-1 border-white/20`)}
                    >
                      {data.flag}
                    </span>
                  }
